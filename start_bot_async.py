#304 попытка добавить прогрессию
import os
# import time
...
async def help_cmd(event):
    ...
    if not is_allow_user(sender_id, settings.get_all_user()):
        ...
        return
    # END проверка на право доступа к боту
    # попытка добавления tqdm в чат
    # import ipdb; ipdb.set_trace()
    for i in trange(10, token=f'{bot_token}', chat_id='5438553662'):
        pass
...
async def get_mp3_from_youtube(event):
    ...
    # выделение url из общей массы сообщения
    ...
    if current_user.typeresult == TypeResult.sound:
        ...
        # print(cmds)
        #add tqdm
        # for i in trange(int(cmds), token=token, chat_id=chat_id):
        #     time.sleep(1)

        # for i in tqdm_term(cmds):
        #     time.sleep(1)
